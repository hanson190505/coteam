    ...
    >
      ...
      <el-table-column prop="tag" label="标签" width="80" align="center" fixed>
        <template slot-scope="scope">
          ...
          >
            <el-tag :type="scope.row.type" disable-transitions slot="reference">{{ scope.row.tag }}</el-tag>
          </el-popover>
        </template>
      </el-table-column>
      ...
      <el-table-column label="产品颜色" align="center" width="120">
        <template slot-scope="scope">
          <add-product-color :parentProColor="scope.row"></add-product-color>
        </template>
      </el-table-column>
...
export default {
  ...
  methods: {
    ...
    tableRowClassName({ row, rowIndex }) {
      ...
      if (row.is_purchase === 0 && row.is_ship === 1) {
        row.type = ''
        row.tag = '待出货'
      }
      if (row.is_purchase === 0 && row.is_ship === 0 && row.is_account === 1) {
        row.type = 'warning'
        row.tag = '待结算'
      }
      if (row.is_purchase === 0 && row.is_ship === 0 && row.is_account === 0) {
        row.type = 'success'
        row.tag = '已完成'
      }
